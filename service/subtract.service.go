...
func SubtractImages() *image.Image {
		...
		go func(startX, endX int) {
			...
			for x := range xLen {
				for y := range yLen {
					...
					newR := uint8(max(float64(ogColorOne.R)-float64(ogColorTwo.R), 0))
					newG := uint8(max(float64(ogColorOne.G)-float64(ogColorTwo.G), 0))
					newB := uint8(max(float64(ogColorOne.B)-float64(ogColorTwo.B), 0))
