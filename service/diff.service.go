		...
		go func(startX, endX int) {
			...
			for x := range xLen {
				for y := range yLen {
					...
					subOneR := uint8(max(float64(ogColorOne.R)-float64(ogColorTwo.R), 0))
					subOneG := uint8(max(float64(ogColorOne.G)-float64(ogColorTwo.G), 0))
					subOneB := uint8(max(float64(ogColorOne.B)-float64(ogColorTwo.B), 0))

					subTwoR := uint8(max(float64(ogColorTwo.R)-float64(ogColorOne.R), 0))
					subTwoG := uint8(max(float64(ogColorTwo.G)-float64(ogColorOne.G), 0))
					subTwoB := uint8(max(float64(ogColorTwo.B)-float64(ogColorOne.B), 0))

					newR := uint8(min(subOneR+subTwoR, 255))
					newG := uint8(min(subOneG+subTwoG, 255))
					newB := uint8(min(subOneB+subTwoB, 255))
