...
func AverageImages() *image.Image {
		...
		go func(startX, endX int) {
			...
			for x := range xLen {
				for y := range yLen {
					...
					newR := uint8(min(min(float64(ogColorOne.R)+float64(ogColorTwo.R), 255)/2, 255))
					newG := uint8(min(min(float64(ogColorOne.G)+float64(ogColorTwo.G), 255)/2, 255))
					newB := uint8(min(min(float64(ogColorOne.B)+float64(ogColorTwo.B), 255)/2, 255))
